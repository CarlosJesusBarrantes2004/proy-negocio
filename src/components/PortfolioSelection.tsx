          ...
          {selectedProject && (
            ...
            >
              <button
                ...
                onClick={() => setSelectedProject(null)}
                className="absolute top-6 right-6 z-50 text-white transition-transform hover:rotate-90 hover:cursor-pointer"
              >
                <X size={40} />
              </button>
              ...
              <div className="relative flex h-full w-full max-w-6xl items-center justify-center">
                ...
                {getImages(selectedProject).length > 1 && (
                  <button
                    onClick={handlePrev}
                    aria-label="Anterior"
                    className="absolute left-2 z-50 rounded-full bg-white/10 p-4 text-white backdrop-blur-md transition-all hover:scale-110 hover:cursor-pointer hover:bg-white/20 md:-left-12"
                  >
                    <ChevronLeft size={32} />
                  </button>
                )}
                ...
                {getImages(selectedProject).length > 1 && (
                  <button
                    ...
                    className="absolute right-2 z-50 rounded-full bg-white/10 p-4 text-white backdrop-blur-md transition-all hover:scale-110 hover:cursor-pointer hover:bg-white/20 md:-right-12"
                  >
  ...
  return (
    ...
    >
      ...
      <div className="absolute right-0 bottom-0 left-0 translate-y-6 transform p-8 transition-transform duration-300 group-hover:translate-y-0">
        ...
        <h3 className="mb-2 text-3xl font-black text-white drop-shadow-lg">
          {project.title}
        </h3>
      ...
      {images.length > 1 && (
        <div className="absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 gap-1.5 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          {images.map((_: any, i: number) => (
            <div
              key={i}
              className={`h-1 rounded-full transition-all duration-300 ${i === currentImg ? "w-6 bg-blue-500" : "w-1.5 bg-white/40"}`}
            />
          ))}
        </div>
      )}
